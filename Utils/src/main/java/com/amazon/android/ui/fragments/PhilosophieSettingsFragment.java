...
/**
 * Provides fragment to be shown when "Philosophie" settings item is clicked.
 */
public class PhilosophieSettingsFragment extends ASettingsFragment {

    private static final String TAG = PhilosophieSettingsFragment.class.getSimpleName();
    ...
    private SingleViewProvider getSingleViewProvider(Context context) {
        ...
        try {
            content = Helpers.getContentFromFile(context,
                                                 context.getString(R.string.philosophie_file));
        }
        catch (Exception e) {
            Log.e(TAG, "could not read philosophie file", e);
        }
