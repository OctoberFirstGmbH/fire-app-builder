...
/**
 * Provides fragment to be shown when "Broadcasters" settings item is clicked.
 */
public class BroadcastersSettingsFragment extends ASettingsFragment {

    private static final String TAG = BroadcastersSettingsFragment.class.getSimpleName();
    ...
    private SingleViewProvider getSingleViewProvider(Context context) {
        ...
        try {
            content = Helpers.getContentFromFile(context,
                                                 context.getString(R.string.broadcasters_file));
        }
        catch (Exception e) {
            Log.e(TAG, "could not read broadcasters file", e);
        }
